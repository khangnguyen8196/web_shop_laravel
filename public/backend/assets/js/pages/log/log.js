...
pages = $.extend(pages, {
    log: {
        init: function () {
            ...
            // list page
            if ($("#log_tbl").length > 0) {
                pages.log.initDatatable();
            }
            $(document).on('click', '.btn_execute_search', function (evt) {
                pages.common.executeSearchForm("search-form", 'log_tbl');
            });
            ...
            $(document).on('click', '#delete-modal .confirm_delete', function (evt) {
                ...
                var selectedStatus = $(this).attr('selected-status');
                var type = 'status';
                pages.log.updateInfo(recordId, selectedStatus, type);
